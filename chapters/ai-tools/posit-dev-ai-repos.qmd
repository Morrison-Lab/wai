...
| [`mcptools`](https://github.com/posit-dev/mcptools) [@posit_dev_mcptools] | Model Context Protocol for R: expose a running R session as an MCP server, or register MCP servers with `ellmer` chats | R | 194 | 2026-08-22 | Yes: lets Claude Code inspect a live R session |
...
| [`mcp-repl`](https://github.com/posit-dev/mcp-repl) [@posit_dev_mcp_repl] | Sandboxed MCP server that keeps a persistent R or Python session alive across an agent's tool calls, with plots returned as images | Rust | 65 | 2026-07-27 | Yes, worth trying: replaces `Rscript -e` round trips in Claude Code |
...
`ellmer`, the R package that `btw`, `mcptools`, `querychat`, and `shinychat` all build on,
and `ragnar`, its retrieval companion,
...
#### Useful to us? Yes, three packages and one reference
...
- **`mcptools`** is the layer underneath, and is worth knowing on its own
...
- **`mcp-repl`** addresses a cost the lab already pays:
  an agent that runs `Rscript -e` for every check reloads data and packages each time.
...
- **`bluffbench2`** is a reference rather than a dependency.
  It is an evaluation written in R with `vitals`
...
`querychat`, `shinychat`, and `commons` matter only if the lab ships Shiny apps
with a chat interface,
and `commons` is still marked experimental.
