...
and @sec-ai-coding-agent-platforms lists the platforms the lab meets most often.
...
so the scaffolding around the model
(the loop, the tools, memory, permissions, and context management)
now does most of the differentiating.
...
and the answer offered is always the same word: the harness.
...
- **Your own tasks are the benchmark that matters.**
  ...
  which is why the recommendations below end in a pilot rather than a ranking.

#### Terminal-first harnesses

| Harness | License | Character (August 2026) |
|---|---|---|
| [Claude Code](https://code.claude.com/docs) (Anthropic) | Proprietary | The deepest programmable harness: hooks, skills, plugins, subagents, MCP, and multi-agent workflows. Widely rated first on code quality, and consistently the heaviest token spender, with real volume starting on the higher subscription tiers. |
...
| [Gemini CLI](https://github.com/google-gemini/gemini-cli) (Google) | Apache 2.0 | Being folded into Antigravity CLI, with free individual access reported to end in mid-2026. |
...
| Harness | License | Character (August 2026) |
|---|---|---|
| [Cursor](https://www.cursor.com/) | Proprietary | The in-editor leader: a VS Code fork with a deep agent loop, an in-house model tuned for fast agentic editing, and rules, MCP, hooks, skills, plugins, and subagents. Reads `.cursor`, `.claude/agents`, and `.codex/agents` configuration. |
...
| Devin Desktop (Cognition) | Proprietary | The former Windsurf editor, rebranded in mid-2026 with a local Devin agent, an agent command center, and Agent Client Protocol support. |
| Continue, Tabnine, Kilo Code, Zed | Mixed | Specialists: Continue adds an open-source agent to an existing editor; Tabnine is the on-premises, air-gapped enterprise pick; Kilo Code carries on the Roo Code lineage after that project shut down; Zed is an open-source AI-native editor. |
...
In its author's own Terminal-Bench 2.0 run it placed second,
...
rather than waste.
Even the minimalists concede that task queues, orchestration, and durable sessions will matter more over time,
so the two camps may converge.

#### Open source versus proprietary

- **Open source:** OpenCode, Codex CLI, Cline, Aider, OpenHands, Goose, Continue, Gemini CLI, Pi, and Zed.
  You pay only your model provider, or run local models.
- **Proprietary:** Claude Code, Cursor, GitHub Copilot, Devin, Amp, Antigravity, Replit Agent, and Tabnine.
...
and that multi-agent designs add real cost and debugging complexity,
