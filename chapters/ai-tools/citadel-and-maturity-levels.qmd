...
[Customizing an Agent](agent-customization.qmd#sec-ai-customization) maps those mechanisms one at a time.
...
Citadel is a plugin, not a harness ([harnesses](agent-architecture.qmd#sec-ai-harnesses)).
...
A skill is a `SKILL.md` protocol file in the format [Agent Skills](agent-customization.qmd#sec-ai-agent-skills) describes,
...
[Customizing an Agent](agent-customization.qmd#sec-ai-customization) makes the point that an instruction file is context the model may ignore,
...
described in [customizing an agent](agent-customization.qmd#sec-ai-customization)'s worked example.
