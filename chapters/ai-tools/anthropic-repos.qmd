...
| `claude-plugins-official` | Anthropic-curated plugin marketplace | Python | 36.1k | 2026-09-09 | Apache-2.0 | Install from it; see [useful plugins](agent-customization.qmd#sec-ai-useful-plugins) |
...
- **`.devcontainer/`** is the container Anthropic uses
  ...
  [firewall configuration](coding-agents.qmd#sec-ai-firewall) discusses.
...
[Claude Code Action review](pr-workflow-with-agents.qmd#sec-ai-claude-code-action-review) already explains
...
in [benefits and hazards](coding-agents.qmd#sec-ai-benefits-hazards).

#### The extension and SDK repositories, briefly

`skills` is the reference implementation of the Agent Skills format
that [Agent Skills](agent-customization.qmd#sec-ai-agent-skills) describes.
...
built for Claude Cowork ([Claude Cowork](grok-bot-and-alternatives.qmd#sec-ai-collaborative-workspaces))
...
- **Adopt now.**
  ...
  and [review](pr-workflow-with-agents.qmd#sec-ai-claude-code-action-review);
...
- **Trial on Linux and macOS.**
  ...
  firewall configuration in [firewall configuration](coding-agents.qmd#sec-ai-firewall)
