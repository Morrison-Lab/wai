...
In contrast, the [Google AI Studio Projects Dashboard](https://aistudio.google.com/projects) lists projects by their **friendly display names** (such as `ai-config Project` or `gha-project`) and alphanumeric client IDs (`gen-lang-client-...`).
...
- **Right-Size Model Selection**: For general Gemini API, Python SDK, or custom scripts, default to `gemini-2.5-flash` instead of `pro` models. `gemini-2.5-flash` costs roughly 15x–30x less per token than `pro` models. For Antigravity Agent workflows (`google-antigravity`), the agent defaults to `gemini-3.7-flash` (already a Flash-tier model). The supported Antigravity Agent model options are `gemini-3.7-flash`, `gemini-3.6-flash`, `gemini-3.5-flash`, and `gemini-3.5-flash-lite`.
- **Use Context Caching**: For workloads that repeatedly pass static context over 32,000 tokens (such as large reference docs or system prompts), use Gemini Context Caching. Cached tokens receive a 75%–85% price reduction.
