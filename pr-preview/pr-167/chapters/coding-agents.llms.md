...
Last modified: 2026-09-01 08:50:10 (PDT)
...
A plugin manifest (such as `plugin.json` or `manifest.json`) orchestrates four distinct architectural components:
...
- **Lifecycle hooks (`hooks/**`)**: Deterministic executable scripts attached to agent lifecycle events (such as `PreToolUse` command inspection, `Stop` review-gate verification, and `PreInvocation` context injection).
...
#### Comparison: Skills vs. MCP Tools vs. Plugins

| Dimension | Skills (`SKILL.md`) | MCP Tools | Plugins (`plugin.json`) |
|----|----|----|----|
| **Primary purpose** | Procedural guidance & workflows | External tool execution & data access | Unified packaging & distribution |
| **Execution model** | Read by LLM into system prompt | Executed by agent harness over IPC | Discovered & loaded by agent platform |
