...
- **Spell check**: words go in `inst/WORDLIST` (see `.github/workflows/check-spelling.yaml`). Update the wordlist instead of disabling the check. The workflow runs two passes: the package check (`README`, `DESCRIPTION`, `man/`) and `.github/scripts/spellcheck-chapters.R` over `index.qmd`, `appendix-*.qmd`, and `chapters/**/*.qmd`. Run that script locally before pushing prose; backtick file paths and code identifiers rather than adding them to the wordlist.
