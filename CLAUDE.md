...
- **Spell check**: words go in `inst/WORDLIST` (see `.github/workflows/check-spelling.yaml`). Update the wordlist instead of disabling the check. The workflow runs two passes: the package check (`README`, `DESCRIPTION`, `man/`) and `.github/scripts/spellcheck-chapters.R` over `index.qmd`, `appendix-*.qmd`, `chapters/**/*.qmd`, and `shared/**/*.md`. Run that script locally before pushing prose; backtick file paths and code identifiers rather than adding them to the wordlist. Add words to `inst/WORDLIST` by hand: `spelling::update_wordlist()` rebuilds the file from the package check alone and drops every chapter-only word.
